...
void bfd_build_packet(uint8_t diag, uint8_t state, uint8_t detect_mult, uint8_t length,
    ...
    /* 
     * Detection time multiplier: The negotiated transmit interval, multiplied by this value, provides the Detection Time for the
    ...
    /* Your discriminator: the discriminator value received from the corresponding remote system. Zero if unknown. */
    packet->your_discr = htonl(your_discr);

    /* 
     * Required min TX interval: minimum interval, in microseconds, that the local system would like to use when 
     * transmitting BFD Control packets,less any jitter applied (see section 6.8.2).
     */
    packet->req_min_tx_interval = htonl(req_min_tx_interval);

    /*
     * Required min RX interval: minimum interval, in microseconds, between received BFD Control packets that this system is 
     * capable of supporting, less any jitter applied by the sender (see section 6.8.2).  If this value is zero, 
     * the transmitting system does not want the remote system to send any periodic BFD Control packets.
     */
